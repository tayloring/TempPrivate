package com.whu.Gongyinchao.schoolservice.chatmodule.adapter;
...
import com.whu.Gongyinchao.schoolservice.chatmodule.util.TimeUtil;
...
public class RecentAdapter extends BaseAdapter {
	...
	public RecentAdapter(Context context, LinkedList<RecentItem> data,
		...
		mMessageDB = SchoolServiceApplication.getInstance().getMessageDB();
		mRecentDB = SchoolServiceApplication.getInstance().getRecentDB();
	}
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		headIV.setImageResource(SchoolServiceApplication.heads[item.getHeadImg()]);
	...
	private CharSequence convertNormalStringToSpannableString(String message) {
		...
		while (localMatcher.find()) {
			...
			if (m - k < 8) {
				if (SchoolServiceApplication.getInstance().getFaceMap()
						.containsKey(str2)) {
					int face = SchoolServiceApplication.getInstance().getFaceMap()
							.get(str2);
