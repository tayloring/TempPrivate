package com.whu.Gongyinchao.schoolservice.chatmodule.xlistview;
...
public class IphoneTreeView extends ExpandableListView implements
	...
	private IphoneTreeHeaderAdapter mAdapters;
	...
	private void headerViewClick() {
		...
		if (mAdapters.getHeadViewClickStatus(groupPosition) == 1) {
			this.collapseGroup(groupPosition);
			mAdapters.onHeadViewClick(groupPosition, 0);
		} else {
			this.expandGroup(groupPosition);
			mAdapters.onHeadViewClick(groupPosition, 1);
		}
	...
	public void setAdapter(ExpandableListAdapter adapter) {
		...
		mAdapters = (IphoneTreeHeaderAdapter) adapter;
	}
	...
	public boolean onGroupClick(ExpandableListView parent, View v,
		...
		if (mAdapters.getHeadViewClickStatus(groupPosition) == 0) {
			mAdapters.onHeadViewClick(groupPosition, 1);
			parent.expandGroup(groupPosition);
			parent.setSelectedGroup(groupPosition);

		} else if (mAdapters.getHeadViewClickStatus(groupPosition) == 1) {
			mAdapters.onHeadViewClick(groupPosition, 0);
	...
	protected void onLayout(boolean changed, int left, int top, int right,
		...
		int state = mAdapters.getTreeHeaderState(groupPos, childPos);
		if (mHeaderView != null && mAdapters != null && state != mOldState) {
			mOldState = state;
	...
	public void configureHeaderView(int groupPosition, int childPosition) {
		if (mHeaderView == null || mAdapters == null
				|| ((ExpandableListAdapter) mAdapters).getGroupCount() == 0) {
			return;
		}

		int state = mAdapters.getTreeHeaderState(groupPosition, childPosition);
		...
		case IphoneTreeHeaderAdapter.PINNED_HEADER_VISIBLE: {
			mAdapters.configureTreeHeader(mHeaderView, groupPosition,
