package com.whu.Gongyinchao.schoolservice.chatmodule.util;
...
public class SendMsgAsyncTask {
	...
	public SendMsgAsyncTask(String jsonMsg,String useId) {
		...
		mBaiduPush = SchoolServiceApplication.getInstance().getBaiduPush();
		mMessage = jsonMsg;
		mUserId = useId;
	...
	// 发送
	public void send() {
		if (NetUtil.isNetConnected(SchoolServiceApplication.getInstance())) {//如果网络可用
			mTask = new MyAsyncTask();
			mTask.execute();
		} else {
			T.showLong(SchoolServiceApplication.getInstance(), R.string.net_error_tip);
		}
	}
